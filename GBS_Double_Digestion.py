...
cmd="bowtie2 -p 8 -x " + FastaPath+".".join(FastFile.split(".")[:-1]) +" -f " + FirsName  + " -S "+ FirsName[:-6]+".sam 2> " +FirsName[:-6]+"_stats.txt"
os.system(cmd)
cmd="bowtie2 -p 8 -x " + FastaPath+".".join(FastFile.split(".")[:-1]) +" -f " + Seconame  + " -S "+ Seconame[:-6]+".sam 2> " +Seconame[:-6]+"_stats.txt"
os.system(cmd)
...
Gsize = sp.getoutput('grep -v ">" '+FastaPath+FastFile +" | wc -c ")
...
for fasta in glob.glob("*"+str(FastaPath+FastFile).split(".")[-1]):
    if fasta==FastFile:
        pass
    else:
        os.system("rm " + fasta)
